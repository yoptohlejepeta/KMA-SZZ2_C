...
with tab1:
   ...
   indicator = col2.selectbox("Ukazatel", ["Vše"] + data["Indicator"].unique().tolist())
   
   if (data_country == "Vše") and (indicator == "Vše"):
       st.dataframe(data, use_container_width=True)
   elif indicator == "Vše":
       st.dataframe(data.loc[data["Country"] == data_country], use_container_width=True)
   elif data_country == "Vše":
       st.dataframe(data.loc[data["Indicator"] == indicator], use_container_width=True)
   else:
       st.dataframe(data.loc[(data["Indicator"] == indicator) & (data["Country"] == data_country)], use_container_width=True)
...
with tab2:
    ...
    col3.subheader("Posledních 10", help="Zobrazeny jsou pouze státy, kde byla dostupná data")
    ...
    tab_hdp, tab_lifeexp, tab_schoolexpm, tab_schoolexpf = st.tabs(["HDP", "Očekávaná průměrná délka života", "Průměrná délka školní docházky - Muži", "Průměrná délka školní docházky - Ženy"])
    
    with tab_hdp:
        st.plotly_chart(graphs.create_bubble_gdp())
    with tab_lifeexp:
        st.plotly_chart(graphs.create_bubble_lifeexp())
    with tab_schoolexpm:
        st.plotly_chart(graphs.bubble_school_exp_male())
    with tab_schoolexpf:
        st.plotly_chart(graphs.bubble_school_exp_female())

with tab3:
    st.header("Čeká Republika")
    st.subheader("Vývoj GPGI v České republice")
    fig = px.line(gpgi_long.loc[gpgi_long["country"] == "Czech Republic"], x="year", y="gpgi", template="none", labels={"year": "Rok", "gpgi":"GPGI"})
    ...
    year = col1.selectbox("Rok",[year for year in range(2016, 2024)], key="asdakjsdnm", help="Rok, který určuje 10 států s nejvyšším GPGI")
